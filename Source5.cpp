#include <unordered_map>//Тип-данных словарь, хранящий ключ и список, указывающий на цепь (цепное хеширование)
#include <iostream>
#include <fstream>
#include <string>
#include <chrono>
...
//Структура записи - Дата рождения: дата, фамилия 
struct DateOfBirth
{
...
void Find(const unordered_map<int, list<HashTableEntry>>& hashTable, DateOfBirth& policy, const string& binary, unsigned long int key, int tableSize)
{
    ...
    if (it != hashTable.end())
    {
        ...
        for (const HashTableEntry& entry : entries)
        {
            if (entry.key == key)
            {
                cout << endl << "День рождения найден в таблице" << endl;
                ...
                if (file)
                {
                    ...
                    {
                        cout << "   День Рождения " << policy.number << endl;
                        cout << "   Фамилия " << policy.surname << endl;
                    }
                }
                file.close();
                return;
    ...
    if (corr == false)
        cout << endl << "День рождения не найден в файле" << endl;
}
...
int main()
{
    ...
    ofstream binaryFileOut(binary, ios::binary);//Бинарный файл ios::binary -  обеспечивает возможность открывать файл для выполнения двоичных операций ввода-вывода
    if (!binaryFileOut)
    {
    ...
    int tableSize = 11;//Размер хеш-таблицы
    ...
    while (flag)
    {
        ...
        case 1:
            ...
            cout << "Введите день рождения, который вы желаете добавить (8 чисел) ";
            cin >> insert;
        ...
        case 2:
            ...
            cout << "Введите день рождения, который вы желаете найти (8 чисел) ";
            cin >> found;
        ...
        case 3:
            ...
            cout << "Введите день рождения, который вы желаете удалить (8 чисел) ";
            cin >> remove;
